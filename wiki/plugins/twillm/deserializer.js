/*\
title: $:/plugins/twillm/deserializer.js
type: application/javascript
module-type: tiddlerdeserializer

Markdown deserializer with YAML frontmatter extraction.

Parses YAML frontmatter delimited by --- markers and maps
extracted values to tiddler fields. Array values on list fields
(tags, list, etc.) are converted to TiddlyWiki bracketed lists.
Non-string, non-array values are stored as their JSON representation.

\*/
"use strict";

var yaml = require("$:/plugins/twillm/yaml.js");

/*
Deserialise a Markdown file with optional YAML frontmatter into tiddler fields.
*/
	...
	if(frontmatter && typeof frontmatter === "object" && !Array.isArray(frontmatter)) {
		...
		for(var i = 0; i < keys.length; i++) {
			...
			if(key === "tags" && result[key]) {
				...
				value = mergeTagValue(result[key], value);
				result[key] = value;
				continue;
			}
			// Convert value to tiddler field string
...
function fieldValueToString(key, value) {
	...
	if(Array.isArray(value)) {
		...
		if($tw.Tiddler.fieldModules[key] && $tw.Tiddler.fieldModules[key].stringify) {
			// Convert array items to strings first
			var stringItems = [];
		...
		// Array on a non-list field → JSON
